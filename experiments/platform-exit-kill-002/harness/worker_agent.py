"""PLATFORM-EXIT-KILL-002 worker agent subprocess.
...
def main() -> None:
    ...
    # APPARATUS REPAIR (disclosed, 002): the driver freezes the plan as an
    # envelope {"plan": [...]}. 001's worker iterated the envelope object
    # directly and crashed before case 1. The worker now reads the attempt
    # list from the envelope. Semantics unchanged.
    plan_envelope = json.loads(Path(args.plan).read_text(encoding="utf-8"))
