"""JEV-WITNESS-VERCEL-001 frozen acceptance cases V1-V5.
...
Scientific contact gate: the live Jev call goes through the vercel-ai-gateway
skill CLI (authd surrogate exchange; the raw key never enters this process).
If the credential is unavailable the CLI exits 2: the harness prints an honest
gate message, performs NO network calls, and exits 2.
...
SKILL_CLI = Path(
    os.environ.get(
        "JEV_VERCEL_EVALUATE_CLI",
        str(Path.home() / "workspace/skills/vercel-ai-gateway/bin/evaluate.py"),
    )
)
...
def live_jev_judgment(request_path: Path) -> tuple[dict, dict]:
    """Call live Jev once through Vercel AI Gateway via the skill CLI.
    ...
    exchange inside the CLI; this process never sees the raw key.
    ...
    if proc.returncode == 2:
        print(
            "JEV-WITNESS-VERCEL-001 acceptance requires the Vercel AI Gateway "
            "credential (Secure Vault connector custom.vercel-ai-gateway).\n"
            "The credential is unavailable in this environment. Per the work "
            "order this is a human gate: the harness does not ask for, "
            "fabricate, or substitute a key. No network calls were made. "
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"provider call failed: {(proc.stderr or '').strip()[:500]}")
    envelope = json.loads(proc.stdout)
    return envelope["body"], envelope["headers"]
...
def main() -> int:
    ...
    try:
        resp_body, resp_headers = live_jev_judgment(request_path)
    ...
    admit_projection(
        view,
        {
            ...
            "projection_id": "jev-witness-vercel-001/v1/active",
            "issuer_id": "operator",
    ...
    admit_projection(
        ...
        {
            "schema": STANDING_PROJECTION_SCHEMA,
            "projection_id": "jev-witness-vercel-001/v1/revoke",
            "issuer_id": "operator",
            "support_hash": support_v1,
            "action_hash": action_hash_v1,
            "standing": "INACTIVE",
    ...
    if v1_committed:
        admit_projection(
            ...
            {
                "schema": STANDING_PROJECTION_SCHEMA,
                "projection_id": "jev-witness-vercel-001/v1/restore",
                "issuer_id": "operator",
                "support_hash": support_v1,
                "action_hash": action_hash_v1,
                "standing": "ACTIVE",
    ...
    return 0 if terminal in ("PASS_JEV_WITNESS_VERCEL_001", "BELOW_RECEIVER_THRESHOLD") else 1
