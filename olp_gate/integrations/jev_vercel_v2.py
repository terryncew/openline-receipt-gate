"""OpenLine Jev Witness (Vercel v2): Jev judgments via Vercel AI Gateway as signed evidence.
...
This module is the JEV-WITNESS-VERCEL-002 adaptation of
:mod:`olp_gate.integrations.jev_vercel` (VERCEL-001 used the AI SDK internal
``/v4/ai/evaluation-model`` route). Science is byte-identical; only the
transport surface changed:

1. Transport route: ``POST https://ai-gateway.vercel.sh/v1/evaluate`` with the
   JSON body ``{"model": "typesafe-ai/jev", "state": ..., "questions": ...,
   "providerOptions": {"gateway": {"zeroDataRetention": true}}}`` (the
...
3. Zero-data-retention: the per-request opt-in ``zeroDataRetention: true`` is
   sent explicitly (the model listing already reports zdr/no_training "all").
...
def build_evaluate_request(
    ...
    *,
    ...
    The zero-data-retention opt-in is sent explicitly per request.
    """
    ...
    return {
        ...
        "questions": questions,
        "providerOptions": {
            "gateway": {"zeroDataRetention": bool(zero_data_retention)}
        },
    }
    ...
    *,
    ...
    payload = {
        ...
        "jev": {
            ...
            "zero_data_retention_requested": True,
...
class JevVercelWitnessPolicy:
    """Frozen receiver policy for the JEV-WITNESS-VERCEL-002 demo.
    ...
    policy_id: str = "jev-witness-vercel-002-demo-policy-v1"
