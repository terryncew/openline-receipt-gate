"""Apparatus qualification for JEV-WITNESS-VERCEL-003 (pre-contact).
...
semantics, v1/evaluate request construction (disallowPromptTraining, NO
zeroDataRetention), generationId request-id extraction, and the honest
zero_data_retention_requested=false evidence binding. No network, no key, no
Jev contact.
...
ACTION = {
    ...
    "content": "jev-witness-vercel-003 marker",
}
ACTION_B = {
    "kind": "write_marker_file",
...
def test_build_evaluate_request_shape():
    ...
    assert "zeroDataRetention" not in body["providerOptions"]["gateway"]
...
def test_gateway_surface_identifiers():
    ...
    assert jev["zero_data_retention_requested"] is False
    assert jev["disallow_prompt_training_requested"] is True
...
def test_policy_id_v2_default():
    assert JevVercelWitnessPolicy().policy_id == "jev-witness-vercel-003-demo-policy-v1"
...
def test_evaluate_commit_high_confidence(tmp_path):
    ...
    assert decision["policy_id"] == "jev-witness-vercel-003-demo-policy-v1"
...
def test_frozen_request_matches_apparatus():
    # The 003 frozen request file must carry exactly what the apparatus
    # sends: disallowPromptTraining true, zeroDataRetention ABSENT.
    exp_dir = Path(__file__).resolve().parents[1] / "experiments" / "jev-witness-vercel-003"
    frozen = json.loads((exp_dir / "harness" / "frozen_request.json").read_text(encoding="utf-8"))
    assert frozen["model"] == JEV_MODEL_ID
    gateway_opts = frozen["providerOptions"]["gateway"]
    assert gateway_opts == {"disallowPromptTraining": True}
    assert "zeroDataRetention" not in json.dumps(frozen)
    # State/questions byte-identical semantics to VERCEL-002's frozen request.
    prev = json.loads(
        (Path(__file__).resolve().parents[1] / "experiments" / "jev-witness-vercel-002"
         / "harness" / "frozen_request.json").read_text(encoding="utf-8")
    )
    assert frozen["state"] == prev["state"]
    assert frozen["questions"] == prev["questions"]
    # And consistent with what build_evaluate_request produces.
    built = build_evaluate_request(frozen["state"], frozen["questions"])
    assert built["providerOptions"] == frozen["providerOptions"]
    assert built["model"] == frozen["model"]
