"""Apparatus qualification for JEV-WITNESS-VERCEL-001 (pre-contact).

All fixtures are SYNTHETIC wire-shaped v4 evaluation payloads, labeled as
such. They qualify the apparatus only: signature, canonicalization, action
binding, policy bands, standing/revocation composition, mutation detection,
and replay semantics. No network, no key, no Jev contact.
"""
...
def _synthetic_response(probability: float = 0.99) -> dict:
    return {
        ...
        "warnings": [],
        "providerMetadata": {"typesafe": {"judgment": "synthetic-fixture"}},
    }
...
def test_build_verify_round_trip():
    ...
    assert receipt["jev"]["evaluated_at_source"] == "witness_clock"
